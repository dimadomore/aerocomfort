...
- **Header:** лого → `/`; меню: Services, Offers, Areas, About, FAQ, Contact; справа — переключатель EN/ES и кнопка-телефон. Моб: бургер + **видимые иконки WhatsApp и телефона** (контакт в один тап уже из шапки, без раскрытия меню). Sticky, компактится при скролле.
...
### 3.1 Home (`/`)
1. 🟦 **Hero — первый экран делает всю работу сразу.** Выше сгиба (и на десктопе, и **особенно на мобильном**) посетитель за ~3 секунды понимает суть и может действовать. Обязательный минимум первого экрана:
   - **Что это** — H1 с услугой: «Air conditioning in Costa Blanca — installation, repair & maintenance».
   - **Где** — регион + ключевые зоны в H1/подзаголовке («Torrevieja, Orihuela Costa & 30+ areas») → закрывает «где работает».
   - **Доверие сразу** — плашка ★5.0 · 160 reviews (Google).
   - **Связь — все каналы на виду:** 🎯 WhatsApp (зелёная кнопка-герой), 🎯 «Get a free quote» (открывает форму/лончер), 🎯 клик-по-телефону. На мобильном WhatsApp и телефон видны ещё и в шапке.
   - 🖼 фото-героя (мастер за работой). 🤖 HVACBusiness.
   - **Сквозные точки контакта, присутствующие уже на первом экране:** плавающий **чат-лончер** (тултип-приветствие + быстрые действия: WhatsApp / позвонить / мини-форма «имя+телефон») и — при лёгком скролле — **липкая нижняя панель** [Позвонить][WhatsApp] на мобильном. Так «где WhatsApp и как оставить контакты» отвечено мгновенно и не уезжает при скролле.
   - Дисциплина: при всей энергичности — один главный визуальный CTA, без визуального шума; ценность и кнопки приоритетнее картинки (на мобильном фото не должно выталкивать кнопки за сгиб).
